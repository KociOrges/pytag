...
def extract_terms(terms_table, pmid, infile, tsv_file):
      """A method to replace the numerical identifiers used from the tagger for the supported entity types with their associated ontology entries.
      The identified terms for a given abstract text are assigned in a list and returned from the method to be saved in the final composite TSV table.
      """
      ...
      if (len(terms_table) > 0 and '?xml version' not in terms_table):
             # Split the table based on newline character to extract the information related to each term #
             ...
             for el in elements:
                # Split the records for each term based on TAB character to obtain its name, entity type and identifier #
                lis = el.split("\t")
                lis_ch_sin = el.split("\t")
                ...
                if(lis_ch != 'False'):
                      lis_ch_sin[0] = lis_ch
                """ Make sure that the list of the annotated terms does not include both plural and singular form for a term and that terms having
                the same identifiers are considered only once. """
                if((lis_ch_sin[0] not in check_dupl_name) and (lis[2] not in check_dupl_id)):
                      ...
                      # Assing the name of the BibTex file with the pmid, the name of the identified term, its ontology entry and its identifier in a string variable #
