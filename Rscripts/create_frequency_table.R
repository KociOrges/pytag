create_frequency_table <- function(data, ontology = "all") {
	...
	onto_terms_freq <- data.frame ( table ( onto_terms_id$Keyword, onto_terms_id$Term ) [,] )
	...
	onto_terms_aggr <- by(onto_terms_freq,INDICES=row.names(onto_terms_freq), FUN=colSums) 
	# collapse the rows with the same name
	freq_table <- as.data.frame(do.call(cbind, onto_terms_aggr))
